...
async def choose_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Отправь мне текст для обработки.", reply_markup=ReplyKeyboardMarkup([["🔄 Сменить режим"]], resize_keyboard=True))
    return TRANSLATING
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔄 Сменить режим":
        await update.message.reply_text("Выбери режим заново:", reply_markup=mode_markup)
        return CHOOSING_MODE
    ...
    if mode == "🇮🇱 Иврит → рукописный":
        ...
        await update.message.reply_text("Пожалуйста, сначала выбери режим с помощью /start.")
        return CHOOSING_MODE
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Ок, пока!", reply_markup=ReplyKeyboardRemove())
...
conv_handler = ConversationHandler(
    ...
    states={
        ...
        TRANSLATING: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message)],
    },
